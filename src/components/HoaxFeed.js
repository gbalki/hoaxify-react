...
const HoaxFeed = () => {
  ...
  useEffect(() => {
    ...
    let looper = setInterval(getCount, 9000);
    ...
    <div>
      {newHoaxCount > 0 && (
        <div
          ...
          style={{ cursor: loadNewHoaxesProgress ? "" : "pointer" }}
      ...
      {!last && (
        <div
          ...
          style={{ cursor: loadOldHoaxesProgress ? "" : "pointer" }}
          onClick={loadOldHoaxesProgress ? () => {} : loadOldHoaxes}
        >
          {loadOldHoaxesProgress ? <Spinner /> : t("Load all hoaxes")}
        </div>
      )}
    </div>
  );
